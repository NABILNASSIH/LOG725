...
class Bat(Actor):
    ...
    def update(self):
        ...
        # Vérifier si move_func est défini
        if self.move_func:
            if self.move_func == self.ai:
                y_movement = self.move_func()  # Ne pas passer self pour ai
            else:
                y_movement = self.move_func(self)  # Passer self pour les autres fonctions
...
class Game:
    ...
    def draw(self):
        ...
        RenderingSystem.draw([self.ball], screen)
...
def update():
    ...
    if state == State.MENU:
        ...
        p1_controls(game.bats[0])
        if num_players == 2:
            p2_controls(game.bats[1])        
        # Has anyone won?
